<template>
  <div class="guide">
    <article>
        ...
        <p>目标网页的编码格式，默认为UTF-8。如果抓取到的数据是乱码，请切换网页编码</p>
        ...
        <p>内置代理使用<a href="https://free-proxy-list.net/">FreeProxyList</a>和<a href="https://www.xicidaili.com">西刺代理</a>可用的代理地址发出请求。</p>
        ...
        <p>(3)因西刺代理质量相对垃圾，所以如果服务器能访问到FreeProxyList，则直接使用其代理，访问不到，使用西刺代理</p>
        ...
        <h3>11.更新间隔(后台管理配置项)</h3>

        <p>两次API调用间隔时间如果在"更新间隔"时间段内，则直接调用数据库数据进行响应，否则将调用爬虫程序进行响应。所有初始生成的API默认更新间隔为0，表示不更新数据，直接从数据库读取数据。</p>
        
        <p>注意根据需求合理配置该项，"更新间隔"配置值较大或者为0，API平均响应速度快，但时效性较差；配置值较小，API平均响应速度慢，但时效性较好。</p>

        <h3>12.标签(后台管理配置项)</h3>

        <p>API标签</p>

        <h3>13.开放权限(后台管理配置项)</h3>

        <p>控制是否将API共享，即在"API Store"面板中看到</p>

        <h3>14.描述信息(后台管理配置项)</h3>

        <p>API描述信息。返回哪些类别数据等等</p>
